...
> Most of the code was AI-assisted; I'm just the PM.

1. Download the YCB dataset from the website
2. Use `obj2mjcf` for convex decomposition (MuJoCo only supports convex meshes for collision detection), and generate the XML files for each object
3. Use `dm_control` to merge the objects with a scene template
...
## Convert YCB dataset to MuJoCo XML (Convex Decomposition)
```
python obj2xml_ycb.py
```
...
## Combine Scene Template with Object (Generate Self-contained XML and Assets)
```
